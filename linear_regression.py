...
def effects_and_overlap(d: pd.DataFrame, label: str):
    rq_hc  = d.loc[d["Group"] == "HC",  "RQ_calc"].astype(float).values
    rq_pd  = d.loc[d["Group"] == "PD",  "RQ_calc"].astype(float).values
    rq_msa = d.loc[d["Group"] == "MSA", "RQ_calc"].astype(float).values
...
def plot_rq_panel(ax, rq_dict, ylabel):
    ...
    ax.bar(x, vals, edgecolor="black", linewidth=1.1,
           color=[colors[g] for g in groups], width=0.75)
...
plot_panel(axes[0, 0], data_mt, comparisons_mt, ylabel="Relative ND1-CN levels (RQ)", log_scale=True, jitter_seed=123)
plot_panel(axes[0, 1], data_tl, comparisons_tl, ylabel="Relative TL (T/S ratio, RQ)", log_scale=True, jitter_seed=456)
